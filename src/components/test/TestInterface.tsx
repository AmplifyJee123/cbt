...
const TestInterface: React.FC<TestInterfaceProps> = ({ 
  ...
  useEffect(() => {
    ...
    const initialSubject = testData.physics?.length ? 'physics' 
                         : testData.chemistry?.length ? 'chemistry'
                         : 'mathematics';
  ...
  const getCurrentQuestion = useCallback(() => {
    ...
    const currentSection = testData[testState.currentSubject][testState.currentSection];
    if (!currentSection?.questions?.length) return null;
    
    return currentSection.questions[testState.currentQuestion];
  }, [testData, testState.currentSubject, testState.currentSection, testState.currentQuestion]);
  ...
  const handleNext = () => {
    ...
    if (testState.currentSection < testData[testState.currentSubject].length - 1) {
      setTestState(prev => ({
        ...prev,
  ...
  const handlePrevious = () => {
    ...
    if (testState.currentSection > 0) {
      const prevSectionQuestions = testData[testState.currentSubject][testState.currentSection - 1].questions.length;
      setTestState(prev => ({
        ...prev,
        currentSection: prev.currentSection - 1,
    ...
    for (let i = currentSubjectIndex - 1; i >= 0; i--) {
      if (testData[subjects[i]]?.length > 0) {
        ...
        const lastQuestionIndex = testData[subjects[i]][lastSectionIndex].questions.length - 1;
  ...
  const handleGoToQuestion = (subject: string, sectionIndex: number, questionIndex: number) => {
    setTestState(prev => ({
      ...prev,
  ...
  const sectionType = testData?.[testState.currentSubject]?.[testState.currentSection]?.type;
  const isMarkedForReview = currentQuestion ? testState.questionStatus[currentQuestion.id]?.markedForReview : false;
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <TestHeader 
        ...
        testData={testData}
        onSubjectChange={(subject) => setTestState(prev => ({...prev, currentSubject: subject, currentSection: 0, currentQuestion: 0}))}
        onSectionChange={(sectionIndex) => setTestState(prev => ({...prev, currentSection: sectionIndex, currentQuestion: 0}))}
      />
