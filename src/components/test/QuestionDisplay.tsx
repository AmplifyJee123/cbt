...
const QuestionDisplay: React.FC<QuestionDisplayProps> = ({
  ...
  useEffect(() => {
    if (sectionType === 'numerical' && selectedAnswer !== undefined && selectedAnswer !== null) {
    ...
    switch (sectionType) {
      ...
      case 'mcq-multiple':
        return {
          ...
          partial: 'Partial marking: +3 (3 correct), +2 (2 correct), +1 (1 correct)',
          color: 'purple'
        };
      case 'numerical':
        return {
  ...
  const handleMcqSelect = (optionIndex: number) => {
    if (sectionType === 'mcq-single') {
      ...
      if (currentSelection.includes(optionIndex)) {
        onAnswer(question.id, currentSelection.filter(i => i !== optionIndex));
      } else {
  ...
  const handleNumericalInput = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleMatchingSelection = (item: string, matchValue: number) => {
  ...
  return (
    ...
    >
      ...
      {scheme && (
        <div className={`mb-6 rounded-lg border bg-${scheme.color}-50 border-${scheme.color}-200 p-4`}>
          <div className="flex items-center justify-between mb-2">
            <h3 className={`font-semibold text-${scheme.color}-800 flex items-center`}>
              {scheme.title}
            </h3>
            <span className={`text-${scheme.color}-700 text-sm font-medium px-3 py-1 bg-${scheme.color}-100 rounded-full`}>
          ...
          {question.options && question.options.map((option: string, index: number) => (
            ...
            >
              ...
              <div className="flex-1">
                <span className="font-medium mr-2 text-gray-800">{String.fromCharCode(65 + index)}.</span>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-3">
              ...
              {question.listI && question.listI.map((item: string, index: number) => (
                <div key={`list1-${index}`} className="flex items-center p-3 bg-gray-50 border border-gray-200 rounded-lg">
                  <span className="font-medium mr-3 text-blue-600">{String.fromCharCode(80 + index)}.</span>
            ...
            <div className="space-y-3">
              ...
              {question.listII && question.listII.map((item: string, index: number) => (
                <div key={`list2-${index}`} className="flex items-center p-3 bg-gray-50 border border-gray-200 rounded-lg">
                  <span className="font-medium mr-3 text-green-600">{index + 1}.</span>
                  <span className="text-gray-700">{item}</span>
                </div>
              ))}
            </div>
          ...
          <div className="bg-amber-50 p-5 rounded-lg border border-amber-200">
            <h3 className="font-medium mb-4 text-amber-800">Match the following:</h3>
            <div className="grid grid-cols-4 gap-4">
              {['P', 'Q', 'R', 'S'].map((item) => (
                <div key={item} className="space-y-2">
                  <div className="font-medium text-center text-blue-600">{item}</div>
                  <select 
                    value={selectedAnswer?.[item] || ''}
                    onChange={(e) => handleMatchingSelection(item, parseInt(e.target.value, 10))}
