...
interface QuestionNavigationProps {
  ...
  questionStatus: any;
  onQuestionSelect: (subject: string, sectionIndex: number, questionIndex: number) => void;
}
...
const QuestionNavigation: React.FC<QuestionNavigationProps> = ({
  ...
  const getStatusColor = (questionId: string) => {
    ...
    if (status.markedForReview && status.answered) return 'bg-purple-600 text-white'; // Marked for review & answered
  ...
  const getStatusCounts = () => {
    ...
    Object.values(questionStatus).forEach((status: any) => {
      counts.total++;
    ...
    <div className="bg-white rounded-lg shadow-md border border-gray-200 h-full flex flex-col">
      ...
      <div className="overflow-y-auto flex-grow p-4">
        <h4 className="font-medium mb-3 text-sm uppercase text-gray-500 border-b pb-2">
          {currentSubject.charAt(0).toUpperCase() + currentSubject.slice(1)} - Section {currentSection + 1}
        </h4>
        
        <div className="grid grid-cols-5 gap-2 mb-6">
          {testData[currentSubject][currentSection].questions.map((question, index) => {
            const isCurrentQuestion = currentSubject === questionStatus[question.id]?.subject &&
            ...
            return (
              <button
                key={question.id}
                onClick={() => onQuestionSelect(currentSubject, currentSection, index)}
                className={`relative w-10 h-10 flex items-center justify-center rounded-md font-medium text-sm shadow-sm hover:shadow-md transition-all transform hover:scale-105 ${
              ...
              >
                {index + 1}
                {questionStatus[question.id]?.markedForReview && questionStatus[question.id]?.answered && (
        ...
        <div className="mb-6 bg-blue-50 p-3 rounded-lg border border-blue-100">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-700">Not Visited ({statusCounts.notVisited})</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-6 h-6 bg-red-500 rounded"></div>
              <span className="text-sm text-gray-700">Not Answered ({statusCounts.notAnswered})</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-6 h-6 bg-green-500 rounded"></div>
              <span className="text-sm text-gray-700">Answered ({statusCounts.answered})</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-6 h-6 bg-purple-600 rounded"></div>
              <span className="text-sm text-gray-700">Marked for Review ({statusCounts.markedForReview})</span>
            </div>
            <div className="flex items-center space-x-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {testData[subject].map((section, sectionIndex) => (
                      <button
                        ...
                        onClick={() => onQuestionSelect(subject, sectionIndex, 0)}
                        className={`px-3 py-1.5 text-sm rounded-md border transition-all ${
                          currentSubject === subject && currentSection === sectionIndex
