...
import { Clock, Award, BookOpen, AlertCircle, ChevronRight, Download, BarChart2, Calculator, Beaker } from 'lucide-react';
...
const ResultScreen: React.FC<ResultScreenProps> = ({ results, userData }) => {
  if (!results) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="bg-white p-8 rounded-xl shadow-md text-center">
          <AlertCircle className="w-16 h-16 mx-auto text-amber-500 mb-4" />
          <h2 className="text-2xl font-bold mb-2 text-gray-800">Results Unavailable</h2>
          <p className="text-gray-600">There was an issue processing your test results. Please contact support.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-16">
      ...
      <div className="bg-gradient-to-r from-blue-800 to-blue-700 text-white py-8 shadow-md">
        <div className="container mx-auto px-4">
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            <div>
              <p className="text-blue-100">Candidate: <span className="text-white font-medium">{userData.name}</span></p>
              <p className="text-blue-100">Roll Number: <span className="text-white font-medium">{userData.rollNo}</span></p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="flex flex-col items-center justify-center p-6 bg-blue-50 rounded-lg border border-blue-100 transform transition-all duration-300 hover:shadow-md hover:scale-[1.02]">
              <div className="text-4xl font-bold text-blue-700">{Math.round(results.marks)}</div>
              <div className="text-sm text-blue-600 font-medium mt-1">OUT OF {Math.round(results.total)}</div>
            ...
            <div className="flex flex-col items-center justify-center p-6 bg-green-50 rounded-lg border border-green-100 transform transition-all duration-300 hover:shadow-md hover:scale-[1.02]">
              ...
              <div className="text-sm text-green-600 font-medium mt-1">ACCURACY</div>
              <div className="mt-4 text-gray-600 text-sm flex items-center">
            ...
            <div className="flex flex-col items-center justify-center p-6 bg-purple-50 rounded-lg border border-purple-100 transform transition-all duration-300 hover:shadow-md hover:scale-[1.02]">
              <div className="text-4xl font-bold text-purple-700">{results.percentile || "N/A"}</div>
              <div className="text-sm text-purple-600 font-medium mt-1">PERCENTILE</div>
              <div className="mt-4 text-gray-600 text-sm flex items-center">
      ...
      <div className="container mx-auto px-4">
        <h2 className="text-xl font-bold mb-6 text-gray-800 border-b pb-2">Subject Performance</h2>
        
        {/* Subject-wise Performance */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {Object.entries(sectionPerformance).map(([subject, data]) => (
            <div key={subject} className="bg-white rounded-xl shadow-md p-6 border border-gray-200 transform transition-all duration-300 hover:shadow-lg">
              ...
              <div className="flex justify-between items-center mb-4">
                <div className="text-3xl font-bold text-gray-800">{data.score}</div>
                <div className="text-sm text-gray-500 bg-gray-100 px-2 py-1 rounded">Out of {data.total}</div>
              </div>
              ...
              <div className="mt-4 h-2 w-full bg-gray-200 rounded-full overflow-hidden">
                <div className="flex h-full">
                  <div 
                    ...
                    style={{ width: `${(data.correct / (data.correct + data.incorrect + data.unattempted)) * 100}%` }}
                  ></div>
                  <div 
                    className="bg-red-500 h-full" 
                    style={{ width: `${(data.incorrect / (data.correct + data.incorrect + data.unattempted)) * 100}%` }}
                  ></div>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 border border-gray-200 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="border border-gray-200 rounded-lg p-4 bg-gray-50 hover:bg-white hover:shadow-md transition-all duration-300">
              ...
              <div className="text-xl font-bold text-gray-800">{results.avgTimePerQuestion || "00:42"}</div>
            </div>
            
            <div className="border border-gray-200 rounded-lg p-4 bg-gray-50 hover:bg-white hover:shadow-md transition-all duration-300">
              <div className="text-sm text-gray-500 mb-1">Time Spent on Correct Answers</div>
              <div className="text-xl font-bold text-gray-800">{results.timeOnCorrect || "01:12:34"}</div>
            </div>
            
            <div className="border border-gray-200 rounded-lg p-4 bg-gray-50 hover:bg-white hover:shadow-md transition-all duration-300">
              <div className="text-sm text-gray-500 mb-1">Time Spent on Incorrect Answers</div>
              <div className="text-xl font-bold text-gray-800">{results.timeOnIncorrect || "00:47:26"}</div>
            </div>
          </div>
        </div>
